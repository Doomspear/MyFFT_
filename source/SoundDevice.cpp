...
CSoundDevice* CSoundDevice::m_pInst = NULL;
...
//사운드파일을 읽기 위한함수(경로)
HRESULT CSoundDevice::LoadWave(TCHAR* pFileName)
{
	HMMIO	hFile;
...
void CSoundDevice::SoundPlay(unsigned int iIndex, DWORD dwFlag)
{
	if(iIndex < 0 || iIndex > int( m_vecSoundBuff.size() ))
		return;
...
void CSoundDevice::SoundStop(unsigned int iIndex)
{
	if(iIndex < 0 || iIndex > int( m_vecSoundBuff.size() ))
		return;
